...
describe('<ChartTooltip />', () => {
    ...
    beforeEach(() => {
        contextSpy = jest.spyOn(React, 'useContext').mockReturnValue(XYChartContextMock);
    });
    ...
    it('should render two rectangles per data', () => {
        ...
        expect(component.find('rect')).toHaveLength(ChartUtils.getXValues(XYChartContextMock.series).length * 2);
    });
    ...
    it('should not throw when there is only one point in a serie', () => {
        contextSpy.mockReturnValue(XYChartOnePointContextMock);
