...
describe('Option picker', () => {
    ...
    describe('<OptionPicker />', () => {
        ...
        afterEach(() => {
            if (optionPicker.exists()) {
                optionPicker.unmount(); // <-- new
            }
        });
        ...
        it('should display as many <Option /> as there are options in the options prop', () => {
            ...
            expect(optionPicker.find('Option')).toHaveLength(OPTION_PICKER_BASIC_PROPS.options.length);

            optionPicker.setProps(moreOptionsProps);

            expect(optionPicker.find('Option')).toHaveLength(moreOptionsProps.options.length);
        });
